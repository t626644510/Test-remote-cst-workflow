...
`S01-known-main-mode-pso`, revised on 2026-07-10 after the bounded unknown-HOM
frequency-fit experiment was incorporated into the production branch.
...
Canonical workflow and closure refs:

```text
workflow/2-rfgun-hom-antenna
codex/S01-known-mode-pso-closure
```

Both refs identify the same cleaned workflow state. Historical phase history is
retained through the pre-reorganisation backup refs and Git bundle documented
in the project-wide status context.
...
- unknown longitudinal HOM frequencies are fixed by default, but may be fitted
  inside explicit non-overlapping peak-centred windows when
  `pso_fit.frequency_fit.enabled=true`;
...
5. By default, PSO optimizes only unknown HOM amplitudes and Q values.
...
9. Optional `FrequencyFitSettings` changes unknown longitudinal variables from
   `[A, Q]` to `[f, A, Q]` using a symmetric `half_width_hz` bound around each
   detected peak. Overlapping windows fail closed with `overlap_policy: reject`.
10. Frequency fitting never changes the supplied known-mode frequency, Q, or
    R/Q and is currently rejected for transverse fitting.
11. Diagnostics record initial frequencies, fitted frequencies, shifts, and
    effective windows in Hz.
12. Known modes are longitudinal-only in this stage. Transverse known modes are
    intentionally unsupported.

Targeted no-CST regression reached:

```text
.venv\Scripts\python.exe -m pytest tests\workflows\test_workflow2_pso_wake_fit.py -q
43 passed
...
- calibrate or mutate known-mode frequency;
- calibrate known-mode `R/Q`;
- calibrate known-mode Q;
- fit known-mode parameters with PSO;
- fit transverse-mode frequency;
- resolve overlapping unknown-HOM frequency windows automatically;
...
not update the known-mode frequency. `frequency_fit.half_width_hz` applies only
to remaining unknown longitudinal HOM peaks.
...
4. Use unknown HOM peak search above the fundamental, e.g. `freq_min_hz >=
   550e6`.
5. Keep frequency fitting disabled unless the detected peak centre is known to
   need bounded adjustment; when enabled, use physically justified,
   non-overlapping `half_width_hz` windows.
6. Inspect `known_mode_wake`, `unknown_mode_wake`, `residual_wake`, and
   diagnostics.
7. Treat PSO output as diagnostic evidence, not a final scalar pass/fail.

The incorporated frequency-fit option is not a known-mode calibration layer.
If future production work continues, the smallest defensible next step remains
an optional diagnostic calibration layer:
...
Use the canonical workflow branch for development and the closure alias for
...
longitudinal wake-domain diagnostic with fixed known modes and optional bounded
frequency fitting for unknown longitudinal HOM peaks.
