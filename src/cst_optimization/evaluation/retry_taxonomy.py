...
# No-CST helper skeleton 鈥?no runtime wiring, no durable DB, no failure reuse.
...
class RetryTier(int, Enum):
    ...
    Descriptive taxonomy only 鈥?no CST or cleanup functions called.
    """
    NO_RETRY = 0
    TIER_1_RECONNECT = 1
    TIER_2_REBUILD = 2
...
def classify_failure_record(
    ...
    *,
    ...
    4. Known status 鈫?corresponding failure class
    5. Unknown status 鈫?UNSUPPORTED_STATUS
    """
...
def should_escalate_to_probably_infeasible(
    ...
    *,
    ...
    Conservative guard 鈥?returns ``False`` for most inputs.
...
def summarize_retry_classifications(
    ...
    *,
    ...
    Returns a dict mapping action value 鈫?count.  No I/O.
    """
    counts: dict[str, int] = {}
    for rec in records:
