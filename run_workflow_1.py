...
def main() -> None:
    ...
    _logger = logging.getLogger("workflow_1")
    ...
    _logger.info("Params: %d  Objectives: %d  n_initial=%d  n_iter=%d  seed=%d",
                 len(cfg.get("parameters", [])), len(cfg.get("objectives", [])),
    ...
    def _on_evaluation(x_phys, raw_values, penalties, solver_ok, error):
        ...
        if all_finite and _wf_ref:
            ...
            ckpt.mark_completed(idx, raw_values=raw_dict, penalties=pen_dict,
                               solver_ok=solver_ok)
        elif not all_finite:
    ...
    workflow, opt, evaluator = build_workflow_1(cfg, checkpoint_callback=_on_evaluation)
    ...
    print(f"[{stage_name}] Planned: {n_initial} initial + {n_iterations} BO = {n_initial + n_iterations}")
    print("-" * 60)
    ...
    if ckpt.loaded_count > 0:
        ...
        if warm_xy is not None and len(warm_xy[0]) > 0:
            prior_data = warm_xy
            _logger.info("Warm-start from checkpoint: %d prior evaluations", len(warm_xy[0]))
    ...
    def _sigint_handler(signum, frame):
        ...
        if ctrl_c_count[0] >= 2:
            ...
            _os._exit(130)
        print("\nWaiting for current evaluation to finish (Ctrl+C again to force quit)...", flush=True)
