...
def test_pending_draft_cannot_merge_when_review_is_required():
    ...
    with pytest.raises(PriorDraftError, match="unreviewed"):
        merge_draft_prior(base_prior, draft, require_reviewed=True)
...
def test_reviewed_srf_draft_merges_only_supported_prior_fields():
    ...
    merged = merge_draft_prior(base_prior, draft, require_reviewed=True)
    ...
    assert merged["literature_semantics"]["source_evidence"]["required_for_all_nonbaseline_fields"] is True
...
def _semantic_package(
    *,
    ...
    return {
        ...
        "curve_priors": [
            {
                "curve_region": "equator" if regime == "superconducting" else "nose",
                "allowed_curve_types": ["local_nurbs_crown"] if regime == "superconducting" else ["smooth_semicircle_then_reverse_quarter_arc"],
                "preferred_forms": ["local_nurbs_crown"] if regime == "superconducting" else ["smooth_semicircle_then_reverse_quarter_arc"],
                "forbidden_forms": [],
                "source_refs": ["txt1"],
                "confidence": 0.73,
