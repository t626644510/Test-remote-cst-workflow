...
Date: 2026-06-18 (post-Phase-14 cleanup and tolerance tooling)
...
| 12 | Add shared `BaseRunner`, complete WF3 builder chain, and unify retry adaptation |
| 13 | Move `build_workflow_3()` into `rfgun_recovery` and adopt runners for WF1/WF2/WF3 |
| 14 | Remove superseded modules, update architecture docs, and add maintained tolerance campaign tooling |

## Intentional Boundaries

- `rfgun_single_pass` remains a reference-only package without a standalone runner.
- Live CST paths and output directories belong in local configuration or CLI overrides.
- Evaluation databases, campaign outputs, and generated reports are local artifacts and are not committed.
