...
| `src/rf_cem/compiler/` | R2 sole semantic/representation composition boundary, explicit continuity/endpoint policy, real-source adapters, `compile_record.v1` plus v0 compatibility, deterministic no-CST proof bundles and CLI. |
...
This bounded branch starts exactly from R4 canonical merge `8c6bd0be38e8b2bbf5d72c1254413ee6b552defe`; R5 and its live branch are untouched and R5 is `paused_or_deferred_by_user`. TD1 replaces provenance-driven join selection with `boundary_continuity_policy.v0`: internal patches and ordinary semantic RF-wall interfaces default to G1 hard, only explicitly reviewed special discontinuities may use C0 intentional-corner overrides, G2 remains supported, endpoints have a separate contract, and every join records C0 gap/tangent angle/curvature delta plus all three pass states and policy provenance. RF500 now has no real C0 override: both `IrisRegion ↔ NoseRegion` joins use `semantic_interface_default`, require G1, have `intentional_corner=false`, report `tangent_angle_deg=0.0` and pass G1 without changing the 2° tolerance. A 0.1 μm endpoint tangent anchor, below the existing 1 μm spline-approximation tolerance, corrects the actual fit endpoint direction. The current immutable no-CST R2 proof is `analysis_outputs/rf_cem_boundary_compiler_td1_td2/r2_boundary_compiler.2980548dcdd5a85e/`, input SHA-256 `2980548dcdd5a85e6719700d7158db6cdfb29c90b4fd44ba9517f444bbbfc365`; SLS-2 compile ID is `sls2.r149.6593e02e.compile.a22fd5f932c1ffff` and RF500 compile ID is `rf500.2c27faee.b1r3.compile.9ad6a32c86b155f6`. The prior v1 `.8f47ca735db8ce8a` and historical v0 `.aa66a3e90125437b` proofs remain unchanged/readable; synthetic C0, explicit G2 and source-native-provenance regressions remain supported.

TD2 makes `SplineApproxRepresentation` / `boundary_representation.v1` canonical with `fidelity=approximate`, `backend_contract=cadquery.splineApprox.v0`, `approximation_tolerance_mm=0.001`, `optimization_ready=true` and `exact_nurbs=false`. Historical `SplineNurbsRepresentation` and v0 payloads remain a deprecated compatibility path and produce equivalent accepted geometry. Workbench names the current implementation as approximate and treats exact NURBS only as planned/not implemented. The distinction among active backend parameters, observation-only trace parameters and `parameter_count` semantics remains an explicit future debt item; this follow-up does not change `parameter_count`, add an optimisation schema or alter spline backend behavior.
...
The current R4 regression proof consumes the two new v1 compiles and stable architecture/source loader: `analysis_outputs/rf_cem_observation_contract_td/r4_observation_contract.dc4d7d12fb9a8c84/`, input SHA-256 `dc4d7d12fb9a8c84e3bfd805402b49d1dd508330c4ea9d6695af1548455616de`. It passes for both instances, 21 descriptor definitions, 240 values, six constraints and 12 evaluations without mutating geometry or defining RF acceptance. The prior `.a0fd43bd4bf4de2f` and original R4 proofs remain immutable. The narrow historical allowlist proves the known historical source identity only; it does not claim current checkout source-byte equivalence and was not expanded in this follow-up.
...
Two consecutive profile rebuilds produced 67 fresh sources, 795 entities and 1539 relations with identical input-set SHA-256 `2f9b1f41b24be06f4799be69eb4346a238fe541a34c0e508e5a328c118d6cef4` and portable snapshot SHA-256 `d286f229e20b1347d32b1da077adfc0401660454b29dcae95ce9a77aef54f8ae`. The restored human-facing inventory includes 30 review entities and 25 semantic entities, including literature and Helper2 records. Metadata binds both current v1 compile IDs, ablation R3 and R4 `.dc4d7d12fb9a8c84`; all 67 indexed sources audit fresh. SQLite bytes are not the contract; the profile, source identities and deterministic portable snapshot are. Workbench roadmap/capability data now reports R0B–R4 `hard_gate_passed_merged`, R5 paused/deferred and TD1/TD2/TD3/Desktop `implemented_no_cst`.
...
two consecutive full-profile rebuilds: 67 fresh sources / 795 entities / 1539 relations
