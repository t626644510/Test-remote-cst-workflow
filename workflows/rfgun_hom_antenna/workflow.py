...
This module is the only owner of ``build_workflow_2`` and the WF2-specific
retry-handler adapter.  The strict shared ``main`` branch exposes only generic
config-to-object helpers.
"""
...
def _build_retry_handler(
    ...
    """Build the retry adapter owned exclusively by Workflow 2."""
    ...
    _logger.info(
        "Workflow 2 retry handler: enabled "
        "(tier1=%d, tier2=%d, tier3=%d, timeout=%.0fs)",
        retry_config.max_tier1,
        retry_config.max_tier2,
        retry_config.max_tier3,
