...
### CST & destructive actions

- **No CST unless explicitly approved.**  Every CST solve consumes a license
  and wall-clock time.  No-CST phases are the default.
- **No destructive / process-kill action unless explicitly approved.**
  Killing orphan DE windows or CST licensing services requires a bounded,
  pre-authorised scenario.
- No runtime / default config changes unless explicitly scoped to the phase.

### Evidence sources

- **No JSONL / Excel as evidence source unless explicitly scoped.**
...
paths (`/tmp/`, `C:\temp\`, or an explicit scratch directory) for DB / live
outputs.
...
stated** in the phase brief and approved by the operator.  Default is
always the strictest interpretation.
