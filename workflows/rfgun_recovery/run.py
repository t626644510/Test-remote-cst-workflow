...
Usage as module::
    ...
    python -m workflows.rfgun_recovery.run --resume-from runs/workflow3/stage_2
"""
...
def _setup_runtime_logging(cfg: dict) -> str:
    output_dir = cfg.get("logging", {}).get("output_dir", "runs/workflow3")
...
def _make_stage_config(
    ...
    base_output = cfg.get("logging", {}).get("output_dir", "runs/workflow3")
...
def main() -> None:
    ...
    try:
        ...
        if resume_path:
            ...
            _logger.info("Resume mode from %s -skipping staged search", resume_path)
