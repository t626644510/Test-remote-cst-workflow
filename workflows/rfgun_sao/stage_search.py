...
class StageCandidateStatus(str, Enum):
    """Taxonomy for per-candidate evaluation outcomes in a stage."""
    COMPLETED = "completed"
...
class StageObservation:
    """One candidate evaluation within a stage.
    ...
    objective_value : float or None
        Weighted scalar (or None if evaluation failed).
    gate_pass : bool or None
        ``True`` if all gates passed, ``False`` if any rejected, ``None`` if
        measurement did not run.
    calibration_pass : bool or None
    solver_ok : bool or None
    retry_attempts : int
        Number of retries consumed for this candidate.
    reused : bool
        Whether this result came from a database hit (no new CST solve).
    error : str
