...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--tolerance-scale", type=_positive_float, nargs="+", default=None,
...
def _apply_config_overrides(cfg, *, n_samples=None, output_dir=None) -> None:
    """Apply CLI overrides while keeping output and DB paths consistent."""

    if n_samples is not None:
        cfg.max_samples = n_samples
    if output_dir is not None:
        cfg.output_dir = output_dir
        cfg.db_path = str(Path(output_dir) / "tolerance_eval.db")
