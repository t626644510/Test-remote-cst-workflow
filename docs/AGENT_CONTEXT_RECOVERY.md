...
The one default profile is intentionally the complete human-visible recipe. It must include the frozen SLS-2 literature semantics and review session together with W1–W4 proofs; do not recover it by creating core/full variants, deleting literature/review sources or adding a selector. A healthy current rebuild reports 67 fresh sources, 826 entities and 1605 relations with input-set SHA-256 `3e4f5fcaa9e0e6e65237b693f490f786e227a736ee8fa5374b02de1389ccd585`.
...
For current W2, use `analysis_outputs/rf_cem_boundary_compiler_td1_td2/r2_boundary_compiler.24bd2492658ad567/` and validate both `compile_record.v2` files with that bundle as `--bundle-root`. It rechecks explicit continuity policy, compiler-generated `CurveRealizationConstraint` entries, `cadquery.spline.tangent_constrained.v0` enforcement provenance, kernel-read actual edge endpoints/tangents, source-trace fidelity, strict identity and all artifact sizes/hashes. RF500 must have zero real intentional corners; both Iris↔Nose interfaces must remain semantic-default G1, use `measurement_basis=kernel_realized_edge`, prove the requested endpoint constraint was applied, and pass without enlarging the 2° tolerance. The pre-kernel secant estimate is diagnostic only and must not decide `required_pass`. The anchor-era v1 proof `.2980548dcdd5a85e`, previous v1 `.8f47ca735db8ce8a` and historical v0 `.aa66a3e90125437b` are compatibility fixtures and must not be changed. If validation fails, do not hand-edit JSON/STEP, add micro-anchors, restore a real C0 override, weaken continuity/tolerance, overwrite a target or delete an older proof; diagnose compiler constraints, backend enforcement, actual edge readback and fidelity, then intentionally build a fresh content-addressed bundle.
...
For current W4 regression, use `analysis_outputs/rf_cem_observation_contract_td/r4_observation_contract.9e722ec6c8b003cb/` and run `python -m rf_cem.observation validate --bundle <that-directory>`. It binds the current v2 compiles and stable architecture/source loader, re-hashes 11 sources/25 artifacts and rechecks both instances plus exact/shape/registry/bundle/constraint/evaluation identities. The previous `.dc4d7d12fb9a8c84`, `.a0fd43bd4bf4de2f` and historical `.d06695921d941eee` proofs remain unchanged. Their exact canonical bundle/path/hash/size compatibility entries prove known historical source identity, not current-checkout source-byte equivalence; arbitrary mismatches still fail closed and the allowlist must not be broadened as a recovery shortcut. If a code/document input or proof is stale, do not edit JSON, patch SQLite, overwrite a proof, loosen units/tolerances or substitute samples for exact geometry; stabilize the source first and build a new content-addressed bundle.
