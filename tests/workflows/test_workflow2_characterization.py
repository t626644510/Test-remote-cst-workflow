...
class TestSolverTimeoutSource:
    ...
    def test_actual_timeout_is_7200_via_real_config(MockCST):
        """Load the package-local WF2 config, apply runner merge
        ...
        cfg_path = (
            _TEST_DIR.parent.parent
            / "workflows"
            / "rfgun_hom_antenna"
            / "config.yaml"
        )
        assert cfg_path.exists(), f"WF2 config not found at {cfg_path}"
    ...
    def test_workflow2_timeout_intent_is_7200():
        """Assert that the package-local WF2 config contains
...
class TestBuildWorkflow2ReturnSignature:
    ...
    - no concrete builder in ``cst_optimization.factory`` (strict boundary)
...
class TestCompletedEvaluationCheckpoint:
    ...
    def test_one_evaluation_creates_one_completed_checkpoint(
        ...
        from workflows.rfgun_hom_antenna.workflow import build_workflow_2 as factory_build
