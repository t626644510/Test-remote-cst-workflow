"""Workflow 3: single-project recovery optimisation for the X-band gun.

This workflow is derived from the tolerance-analysis project background:
we already have a reasonably good baseline geometry, but after adding
extra structure the resonant frequency, coupling, field flatness, and
input power have shifted away from their design values.

Key behaviour
-------------
1. Evaluate the baseline point first.
2. Use a two-pass CST solve:
   - pass 1: calibrate the true resonant frequency
   - pass 2: re-solve with ``f_data`` corrected to that resonance
3. Reject candidates early when the calibrated resonance drifts too far
   from the target frequency.
4. Separate metric roles:
   - ``gate``: early rejection logic
   - ``optimize``: enters SAO / SAEA optimisation targets
   - ``threshold``: soft engineering limits
   - ``report_only``: recorded for diagnostics, not optimised
"""
...
from ..physics.poynting import discover_field_files, max_modified_poynting
...
@dataclass
class MetricSpec:
    """Configuration for one workflow-3 metric."""
...
class EvaluationStatus(Enum):
    """Classification of a workflow-3 evaluation outcome."""
...
class EvaluationResult:
    ...
    def solver_ok(self) -> bool:
        """Backward-compatible alias — True only when status is SUCCESS."""
        return self.status == EvaluationStatus.SUCCESS

    @solver_ok.setter
    def solver_ok(self, value: bool) -> None:
        # no-op setter for backward compat with code that assigns solver_ok directly
        pass
